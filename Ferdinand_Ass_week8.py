...
for i in range(50):
  ...
  for phone_box in second_search:
    ...
    imp_details = phone_box.find("a", attrs = {"class": "core"})
    ...
    try:
      catch_apostrophe = spec_of_phone.split("'")
      spec_new = catch_apostrophe[0] + catch_apostrophe[1]
      specification_list.append(spec_new)
    except:
